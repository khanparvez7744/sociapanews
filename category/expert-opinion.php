...
<script>
  ...
  <meta name="title" content="Sociapa News | Analysis">
  <meta name="keywords" content="Sociapa News | Analysis">
  <meta name="description" content="Sociapa News | Analysis">
  <meta name="author" content="Sociapa News | Analysis">
  <title>Sociapa News | Analysis</title>
...
<body>
  ...
  <?php
     $data=0;
     $sql="SELECT * FROM category where category='Analysis' and is_active=0";
     $result=$pdo->query($sql);
     $data=$result->fetch();
     if(empty($data)){
        $result=$pdo->query("SELECT * FROM subcategory where sub_category='Analysis' and is_active=0");
        $data=$result->fetch();
     }
   ?>
  <!--<section>-->
...
<!-- !advertisment area -->
      ...
      <div class="col-lg-8 col-6 px-lg-5 px-0">
        ...
        <div class="row g-3">
          <?php $sql="SELECT * FROM news where (category='Analysis' or sub_category='Analysis') and is_active=0";
           $result=$pdo->query($sql);
          $data=$result->fetchAll();
          foreach ($data as $value) {
              $a = strip_tags( $value['heading']);
